...
export const cafeFlowRequestAiPromoSuggestionsHandler: BffHandler = async ({ ctx }) => {
  // Both inputContract fields (actorId, windowStart) are resolved from ctx inside the usecase
  // (actorSession and systemDefault respectively) — they are NOT public client inputs.
  // The usecase Input type is empty, so no boundary validation is needed.
...
export const routes: ControllerRoute[] = [
  {
    ...
    handler: cafeFlowRequestAiPromoSuggestionsHandler,
  },
];
